...
namespace VoiceToForm.VoiceRecognitionSystem
{
    ...
    public class VoiceToText
    {
        ...
        public VoiceToTextBinder BindToNativeSpeechRecognistion(Entry entry, View clickableView = null, VoiceConvertedToTextDelegate voiceConvertedToTextCallback = null, bool overRideNewSpeechToText = true) 
        {
            VoiceToTextBinder binder = new VoiceToTextBinder(layout, overRideNewSpeechToText);
            if (binders.Count == 0)
            {
        ...
        {
            VoiceToTextBinder binder = new VoiceToTextBinder(layout, true);
            if (binders.Count == 0)
            {
                binder.NoMicrophoneIsAvailable_Notification = NoMicrophoneCallback;
            }
    ...
    public class VoiceToTextBinder { 
        ...
        private bool overrideExistingTextDoNotAppend;
        ...
        //Constructor
        public VoiceToTextBinder(StackLayout layout, bool overrideText)
        {
            this.layout = layout;
            this.overrideExistingTextDoNotAppend = overrideText;
        }
        ...
        private void SetupAndroidVoiceButton()
        {
            ...
            androidVoiceButton.OnTextChanged += (s) => {
                if (amTheActiveVoiceToTextListner)
                {
                    ...
                    if (!overrideExistingTextDoNotAppend && speechToTextEntry.Text.EndsWith(" ") == false)
                    {
                        s = " " + s;
                    }
                    if (speechToTextEntry != null)
                    {
                        speechToTextEntry.Text = this.overrideExistingTextDoNotAppend ? s : speechToTextEntry.Text + s;
                    }

                    if (lblSpeecToTextEntry != null)
                    {
                        lblSpeecToTextEntry.Text = this.overrideExistingTextDoNotAppend ? s : lblSpeecToTextEntry.Text + s;
                    }
