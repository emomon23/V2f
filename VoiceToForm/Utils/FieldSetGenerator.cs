...
using VoiceToForm.VoiceRecognitionSystem;
using Xamarin.Forms;
...
namespace VoiceToForm.Utils
{
    public class FieldSetGenerator
    {
        ...
        public Color DefaultLabelColor = Color.Black;
        ...
        public FieldSetViewContainer CreateFieldSet(string fieldSetId, string labelText, string binding, bool includeButtonContainer = false, int floatWithPercent = 0, EventHandler<TextChangedEventArgs> textChangeEventHandler = null)
        {
            var result = CreateFieldSet(fieldSetId, labelText, null, binding, includeButtonContainer, floatWithPercent);

            if (textChangeEventHandler != null)
            {
                ((Entry) result.InputElement).TextChanged += textChangeEventHandler;
            }
            return result;
        }

        public FieldSetViewContainer CreateVoiceToTextFieldSet(string fieldSetId, string labelText, string binding = null, Action<string> voiceToTextConvertedCallback = null, int floatWidthPercent = 0, bool overrideExistingText = true, EventHandler<TextChangedEventArgs> textChangeEventHandler =null)
        {
            ...
            if (textChangeEventHandler != null)
            {
                ((Entry) container.InputElement).TextChanged += textChangeEventHandler;
            }

            var button = ImgBtnGenerator.AddButton(container.ButtonContainer, nullCommand, 60, 28, image: "record1.png");
